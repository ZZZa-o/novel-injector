...
export function niSelectNodeAwareRecallCandidates(candidates, topK, nodeCtx, {
    ...
    for (const chunk of ranked) {
        const sourceChunkIdx = niGetVectorSourceChunkIdx(chunk);
        const sourceKey = sourceChunkIdx == null
            ? `${chunk.stageIdx}:${chunk.chunkIdx}`
            : `${chunk.stageIdx}:source:${sourceChunkIdx}`;
...
export function createVectorRecallService({
    ...
    async function recallRelevantWeighted(weightedQueries, stageList, opts = {}) {
        ...
        const nodeCtx = opts.nodeRecallContext || lightCtx || null;
        ...
        const enabledChunks = allChunks
            .filter(isVectorRowCompatible)
            .filter(chunk => enabledStages.has(chunk.stageIdx));
        try {
            onVectorQueryCompared?.(enabledChunks, combined.length, { novelKey });
        } catch (error) {
            logger.warn('[NI] 更新向量兼容提示失败:', error);
        }

        const candidates = enabledChunks
            .filter(chunk => isVectorDimensionCompatible(chunk, combined.length))
        ...
        const orderedCandidates = nodeCtx
            ? niSelectNodeAwareRecallCandidates(candidates, topK, nodeCtx, { minScore: thresh })
            : niSelectRecallCandidatesInStoryOrder(candidates, topK);
    ...
    async function recallRelevant(queryText, stageList) {
        ...
        const enabledChunks = allChunks
            ...
            .filter(chunk => enabledStages.has(chunk.stageIdx));
        try {
            onVectorQueryCompared?.(enabledChunks, queryVector.length, { novelKey });
        } catch (error) {
            logger.warn('[NI] 更新向量兼容提示失败:', error);
        }
...
export function createVectorController({
    ...
    function niGetVectorStageSelection(key, stageCount) {
        ...
        if (!session) {
            session = { selected: new Set(), knownStages: new Set() };
        ...
        for (let stageIdx = 1; stageIdx <= stageCount; stageIdx++) {
            ...
            if (!S.stageVecDone[stageIdx]) session.selected.add(stageIdx);
        }
        return session;
    }
    ...
    function niBindVecStageSelectionTracking() {
        qa('#ni-vec-stage-selector .ni-vec-stage-chk').forEach(element => {
            ...
            element.addEventListener('change', () => niCaptureVecStageSelection());
        });
        ['#ni-vsp-all', '#ni-vsp-none', '#ni-vsp-pending'].forEach(selector => {
            ...
            button.addEventListener('click', () => Promise.resolve().then(() => niCaptureVecStageSelection()));
        });
    }
    // ============================================================
    // IndexedDB 封装
