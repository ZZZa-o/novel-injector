...
export function niDiffDeviationFacts(before, after, { floor = 0 } = {}) {
...
export function niReconcileDeviationFacts(existing, incoming, {
    ...
    incomingFacts.forEach((incomingFact, index) => {
        ...
        if (old) {
            ...
            nextFacts.push({
                ...
                updatedFloor: textChanged ? (currentFloor ?? incomingFact.updatedFloor ?? old.updatedFloor) : old.updatedFloor,
                confirmedFloor: textChanged ? old.confirmedFloor : (currentFloor ?? incomingFact.confirmedFloor ?? old.confirmedFloor),
                createdFloor: old.createdFloor ?? incomingFact.createdFloor ?? (currentFloor ?? null),
            });
        } else {
            nextFacts.push({
                ...incomingFact,
                status: 'active',
                createdFloor: incomingFact.createdFloor ?? currentFloor,
                updatedFloor: incomingFact.updatedFloor ?? currentFloor,
            });
        }
    });
...
export function niMergeDeviationSections(existing, next, { floor = 0 } = {}) {
