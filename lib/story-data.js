...
export function niMergeDeviationSections(existing, next) {
...
export function niDevMessageRole(message) {
    ...
    if (role === 'system' || message?.extra?.isSmallSys === true) return '[系统]';
    return '[AI]';
}
...
export function createStoryController(deps = {}) {
    ...
    function niMainPickerEntries() {
        const editingMain = _plotEditTarget?.type === 'main'
            ? (S.plots.main || [])[_plotEditTarget.idx]
            : null;
        return niOrderedPlotEntries([{ type: 'main', items: S.plots.main || [] }])
    ...
    function niUpdatePivotMainPickerLabel() {
        ...
        const entries = niMainPickerEntries();
        const selectedIndex = entries.findIndex(entry =>
            `main-id:${niEnsurePlotNodeId(entry._plotRef, 'main', entry._sourceIdx)}` === hidden.value
        );
        if (selectedIndex < 0) {
            label.textContent = _plotModalMode === 'add' ? '不指定（末尾追加）' : '不指定（保留原位置）';
            return;
        }
        const entry = entries[selectedIndex];
        const number = String(selectedIndex + 1).padStart(3, '0');
        label.textContent = `主线 ${number}｜${entry.title || '（无标题）'}`;
    }
    ...
    function niRenderPivotMainPicker(query = '') {
        ...
        const visible = entries.map((entry, index) => {
            ...
            const haystack = [number, index + 1, entry.title, entry.time, entry.location, stageText]
                .map(value => String(value || '').toLocaleLowerCase())
                .join('\n');
            return { entry, number, stageText, matches: !keyword || haystack.includes(keyword) };
        ...
        if (!keyword) {
            html.push(`<button type="button" class="ni-main-picker-option${hidden.value ? '' : ' on'}" data-main-key="" role="option" aria-selected="${hidden.value ? 'false' : 'true'}">
              ...
              <span class="ni-main-picker-option-meta">${_plotModalMode === 'add' ? '将关键转折追加到当前剧情末尾' : '保留关键转折当前所在位置'}</span>
            </button>`);
        }
        let lastStage = null;
        visible.forEach(item => {
            ...
            const key = `main-id:${niEnsurePlotNodeId(item.entry._plotRef, 'main', item.entry._sourceIdx)}`;
            const selected = hidden.value === key;
            const meta = [item.stageText, item.entry.time, item.entry.location].filter(Boolean).join(' · ');
            html.push(`<button type="button" class="ni-main-picker-option${selected ? ' on' : ''}" data-main-key="${niEscAttr(key)}" role="option" aria-selected="${selected ? 'true' : 'false'}">
              <span class="ni-main-picker-option-title">主线 ${item.number}｜${niEscHtml(item.entry.title || '（无标题）')}</span>
              <span class="ni-main-picker-option-meta">${niEscHtml(meta || '暂无时间地点信息')}</span>
            </button>`);
        });
        if (!html.length) html.push('<div class="ni-main-picker-empty">没有找到匹配的主线</div>');
    ...
    function niRefreshPivotAfterMainField(type, pivotPlot = null, resetSelection = false) {
        ...
        if (resetSelection) hidden.value = niFindPreviousMainKeyForPivot(pivotPlot);
